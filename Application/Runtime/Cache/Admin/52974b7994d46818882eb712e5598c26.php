						...
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="long-td">
                                        ...
                                        <td><div class="text-left"><?php echo ($vo["nickname"]); ?>/<strong><?php echo ($vo["phone"]); ?></strong></div></td>
                                        <td><?=$vo['type_id']==1?'用户':'律师';?></td>
                                        <td><?php echo ($vo["amount"]); ?></td>
                                        <td><?php echo (date('Y-m-d H:i:s',$vo["starttime"])); ?>-
                                            <?php echo (date('Y-m-d H:i:s',$vo["deadtime"])); ?></td>
                                        <td><?=$vo['status']==0?'未使用':'已使用';?></td>
                                        <td>
                                            <?php
 if($vo['usetime']){ echo date('Y-m-d H:i:s',$vo['usetime']); } ?>
                                        </td>
										<!-- <td>
											<a href="javascript:;" data-id="<?php echo ($vo["feedid"]); ?>" class="btn-sm btn-danger del" type-id=" <?=$vo['type_id']?>">
    ...
    $(function(){
        $(".del").click(function(){
            ...
            layer.confirm('确定删除？', {icon: 3}, function(index){
                layer.close(index);
                $.post("<?php echo U('delfeed');?>",{feedid:id,type_id:type_id},function(data){
