...
$(document).ready(function(){
	$("#loginout").click(function(){
						...
						<ul class="list-group">
                            <li class="list-group-item">
                                <p class="text-success"><a href="<?php echo U('Index/index');?>" title="返回首页" class="tip-bottom"><i class="fa fa-home"></i> 首页</a> /
                                    <a href="<?php echo U('Admin/Order/list2');?>" class="tip-bottom">订单管理</a> /
        ...
        <div class="row">
            ...
            <div class="col-sm-10">
                <form action="/xj_property/Admin/Order/list2" class="form-inline" method="get">
 ...
 foreach($placeList as $v){ ?>
                    ...
                    <input type="text" class="form-control input-sm" id="stime" placeholder="###&lt;预订时间" name="stime" value="<?php echo ($_REQUEST['stime']); ?>" size="10">
                    </div>
                    <div class="form-group">
                    <label for="title" class="sr-only">预订时间&gt;</label>
                    <input type="text" class="form-control input-sm" id="etime" placeholder="预订时间&gt;###" name="etime" value="<?php echo ($_REQUEST['etime']); ?>" size="10">
                    </div>
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-title">
						<h5><i class="fa fa-tasks"></i> 美容美发订单</h5>
					</div>
						...
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="long-td">
                                        ...
                                        <td>
                                        	<?php if($vo["payway"] == 1): ?>刷卡<?php endif; ?>
                                            <?php if($vo["payway"] == 2): ?>包月<?php endif; ?>
                                        </td>
                                        <td><?php echo (date("Y-m-d H:i",$vo["ctime"])); ?></td>
                                        <td>
                                            <?php if($vo["status"] == 0): ?>排队中<?php endif; ?>
                                            <?php if($vo["status"] == 1): ?>进行中<?php endif; ?>
                                            <?php if($vo["status"] == 2): ?>已完成<?php endif; ?>
                                            <?php if($vo["status"] == -1): ?>已取消<?php endif; ?>
                                        </td>
                                        <td class="btns" style="text-align:center;">
                                        	<a href="javascript:;" class="btn-sm btn-success edit" data-oid="<?php echo ($vo["oid"]); ?>" style='background-color:#1ab394;border-color:#1ab394;color:#FFF;'>订单详情</a>
                                        <?php
 if($vo['status']==2 || $vo['status']==-1){ ?>
                                            <a href="javascript:;" class="btn-sm btn-success del" data-oid="<?php echo ($vo["oid"]); ?>" style='background-color:#ed5565;border-color:#ed5565;color:#FFF;'>删除</a>
                                        <?php
 } ?>                                            
					                    </td>
...
<script>
    ...
    $(function(){
    	...
    	$('.edit').on('click',function(){
    		...
    		location.href='/xj_property/Admin/Order/orderInfo2?oid='+oid;
    	});
        ...
        $('.del').on('click',function(){
        	...
        	layer.confirm('确定删除？', {icon: 3}, function(index){
	            $.ajax({
	                url:'/xj_property/Admin/Order/del2',
	                data:'oid='+oid,
	                type:'post',
