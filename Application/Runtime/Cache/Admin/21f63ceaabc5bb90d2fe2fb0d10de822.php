...
$(document).ready(function(){
	$("#loginout").click(function(){
                                    ...
                                    <ul class="list-group">
                                        <li class="list-group-item">
                                            <p class="text-success"><a href="<?php echo U('Index/index');?>" title="返回首页" class="tip-bottom"><i class="fa fa-home"></i> 首页</a> /
                                                <a href="<?php echo U('Admin/User/index');?>" class="tip-bottom">活动管理</a> /
        ...
        <div class="row">
            <div class="col-sm-1">
                <a href="<?php echo U('addactivity');?>" class="btn btn-primary btn-sm">添加活动</a>
            </div>
        </div>
        <br/>
		<div class="row">
						...
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="long-td">
                                        ...
                                        <td>
                                            <img src="<?php echo ($vo["poster"]); ?>" style="width:90%;max-height: 300px;overflow-y: hidden;"/>
                                        </td>
                                        <td>
                                        <?php echo (date("Y/m/d H:i",$vo["starttime"])); ?>
                                        </td>
                                        <td>
                                           <?php echo (date("Y/m/d H:i",$vo["endtime"])); ?>
                                        </td>
					<td class="btns">
                                            <?php if($vo['isend'] == 0): ?><a href="<?php echo U('addactivity',array('bid'=>$vo['bid']));?>" class="btn-sm btn-primary btnrcmd">
                                                    <i class="fa fa-pencil"></i> 修改
                                                </a><?php endif; ?>
                                                <a href="<?php echo U('applys',array('bid'=>$vo['bid']));?>" class="btn-sm btn-primary">
                                                    参与人
                                                </a>
                                                <a href="javascript:;" data-id="<?php echo ($vo["bid"]); ?>" class="btn-sm btn-primary del">
                                                    删除
                                                </a>
                                        </td>
    ...
    <script type="text/javascript">
        $(".del").click(function(){
            ...
            layer.confirm('确定删除该期活动？', {icon: 3}, function(index){
                $.getJSON('<?php echo U('del','','');?>/bid/'+id,function(data){
