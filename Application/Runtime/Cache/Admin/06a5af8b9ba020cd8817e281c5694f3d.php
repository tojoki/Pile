...
$(document).ready(function(){
	$("#loginout").click(function(){
		...
		<br />
		<a href="<?php echo U('topic/addcat');?>" class="btn btn-primary">添加类型</a>
		<br />
		<br />
		<div class="row">
			<div class="col-sm-12">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="long-td">										
										...
										<td><?php echo (date('Y-m-d H:i:s',$vo["ctime"])); ?></td>
                                                                                <td><?php if($vo['isrcmd'] == '0'): ?>否<?php elseif($vo['isrcmd'] == '1'): ?>是<?php endif; ?> </td>
                                                                                <td><?php echo ($vo["sortval"]); ?></td>
										<td>
											<a href="<?php echo U('addcat',array('catid'=>$vo['catid']));?>" class="btn btn-primary btn-mini">
                                                                                            编辑</a>&nbsp;&nbsp;
                                                                                        <?php if($vo['isrcmd'] == '0'): ?><a href="<?php echo U('rcmdcat',array('catid'=>$vo['catid'],'status'=>1));?>" msg='确认推荐该分类？' class="btn btn-primary btn-mini rcmdbtn">推荐</a>
                                                                                            <?php elseif($vo['isrcmd'] == '1'): ?>
                                                                                            <a href="<?php echo U('rcmdcat',array('catid'=>$vo['catid'],'status'=>0));?>" msg='确认取消推荐？'  class="btn btn-primary btn-mini rcmdbtn">取消推荐</a><?php endif; ?>
                                                                                               &nbsp;&nbsp;
											<a href="javascript:;" onclick="return del(<?php echo ($vo["catid"]); ?>);" class="btn btn-danger">
												删除</a>
										</td>										
		...
		<script type="text/javascript">
                    function del(id){
                        layer.confirm('确认删除该类型？', {icon: 3}, function(index){
                            layer.close(index);
                            $.post("<?php echo U('delcat');?>",{catid:id},function(data){
