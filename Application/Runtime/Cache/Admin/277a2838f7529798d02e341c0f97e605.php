...
$(document).ready(function(){
	$("#loginout").click(function(){
                        ...
                        <ul class="list-group">
                            <li class="list-group-item">
                                <p class="text-success"><a href="<?php echo U('Index/index');?>" title="返回首页" class="tip-bottom"><i class="fa fa-home"></i> 首页</a> /
                                    <a href="<?php echo U('Admin/User/index');?>" class="tip-bottom">平台管理</a> /
                                    <a href="javascript:;" class="tip-bottom">服务类型</a>
                                </p>
                            </li>
                        </ul>
                    </div>
        ...
        <div class="row">
            <div class="col-sm-1">
                <a href="<?php echo U('add1');?>" class="btn btn-primary btn-sm">添加洗车服务</a>
            </div>
        </div>
        <br/>
		<div class="row">
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-title">
						<h5><i class="fa fa-tasks"></i> 洗车服务</h5>
					</div>
					<div class="ibox-content">
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($list1)): $key = 0; $__LIST__ = $list1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr class="long-td">
                                        <td><?php echo ($key); ?></td>
                                        <td>
                                        <?php
 if($vo['cover']){ ?>
                                            <img src="/xj_property/<?php echo ($vo['cover']); ?>" width="100"/>
                                        <?php
 } ?>
                                        </td>
                                        ...
                                        <td><?php echo ($vo["limit_num"]); ?></td>
                                        <td><?php echo ($vo["limit_stime"]); ?>-<?php echo ($vo["limit_etime"]); ?></td>
					                    <td class="btns">
                                            <a href="<?php echo U('add1',array('id'=>$vo['id']));?>" class="btn-sm btn-primary edit" style='background-color:#1ab394;border-color:#1ab394;color:#FFF;'>
                                                <i class="fa fa-pencil"></i> 修改
                                            </a>
                                            <a href="javascript:;" data-id="<?php echo ($vo["id"]); ?>" class="btn-sm btn-danger del del1" style="background-color:#ed5565;border-color:#ed5565;color:#FFF;">
        ...
        <div class="row">
            <div class="col-sm-1">
                <a href="<?php echo U('add2');?>" class="btn btn-primary btn-sm">添加美容美发服务</a>
            </div>
        </div>
        <br/>
        <div class="row">
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5><i class="fa fa-tasks"></i> 美容美发服务</h5>
                    </div>
                    <div class="ibox-content">
                        <table class="table table-bordered">
                            <thead>
                            ...
                            <tbody>
                                <?php if(is_array($list2)): $key = 0; $__LIST__ = $list2;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr class="long-td">
                                        <td><?php echo ($key); ?></td>
                                        <td>
                                        <?php
 if($vo['cover']){ ?>
                                        ...
                                        <td><?php echo ($vo["limit_stime"]); ?>-<?php echo ($vo["limit_etime"]); ?></td>
                                        <td class="btns">
                                            <a href="<?php echo U('add2',array('id'=>$vo['id']));?>" class="btn-sm btn-primary edit" style='background-color:#1ab394;border-color:#1ab394;color:#FFF;'>
                                                <i class="fa fa-pencil"></i> 修改
                                            </a>
                                            <a href="javascript:;" data-id="<?php echo ($vo["id"]); ?>" class="btn-sm btn-danger del del2" style="background-color:#ed5565;border-color:#ed5565;color:#FFF;">
                                                <i class="fa fa-trash-o"></i>删除
                                            </a>
                                        </td>
                                        </tr><?php endforeach; endif; else: echo "" ;endif; ?>
    ...
    <script type="text/javascript">
        ...
        $(".del1").click(function(){
            ...
            layer.confirm('确定删除？', {icon: 3}, function(index){
                $.getJSON('<?php echo U('del_user1','','');?>/id/'+id,function(data){
        ...
        $(".del2").click(function(){
            ...
            var id = thi.data("id");
            layer.confirm('确定删除？', {icon: 3}, function(index){
                $.getJSON('<?php echo U('del_user2','','');?>/id/'+id,function(data){
                    layer.msg(data.msg,{skin:"layui-layer-hui",time:1000,end:function(){
                        if(data.status == 0){
                            window.location.reload();
                        }
