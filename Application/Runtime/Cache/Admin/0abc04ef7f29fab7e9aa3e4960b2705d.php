						...
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($list)): $val = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($val % 2 );++$val;?><tr class="long-td">
										...
										<td><img class="img-circle" src="/xj_property/Uploads/<?php echo ((isset($vo["userimg"]) && ($vo["userimg"] !== ""))?($vo["userimg"]):'userImg.jpg'); ?>" style="width: 70px;height: 70px" /></td>
										<td>
											<?php if(is_array($list2)): $i = 0; $__LIST__ = $list2;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i % 2 );++$i; if($vo["role"] == $vo2["id"] ): echo ($vo2["name"]); endif; endforeach; endif; else: echo "" ;endif; ?>
										</td>
										<td>
											<?php if($vo["state"] == 1 ): ?>正常<?php endif; ?>
											<?php if($vo["state"] == 0 ): ?>停用<?php endif; ?>
										</td>
