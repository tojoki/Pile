						...
						<table class="table table-bordered">
							...
							<tbody>
                                <?php if(is_array($model)): $i = 0; $__LIST__ = $model;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="long-td">
                                        ...
                                        <td title="<?php echo ($vo["order_ids"]); ?>">查看</td>
                                        <td><?php echo ($vo["uname"]); ?></td>
                                        <td><?php echo ($vo["mobile"]); ?></td>
                                        <td><?php echo ($vo["area"]); echo ($vo["detail"]); ?></td>
                                        <td><?php echo date('Y-m-d H:i:s',$vo['ctime']);?></td>
                                        <td>
                                            <?php switch($vo['status']): case "1": ?>待开票<?php break;?>
                                                <?php case "2": ?>待收票<?php break;?>
                                                <?php case "3": ?>已收票<?php break; endswitch;?>
                                        </td>
                                        <td width="20%">
                                            <?php if($vo['status'] == '1'): ?><a href="#" class="btn btn-primary">开票</a>
                                                <input name='invoice_id' type='hidden' value="<?php echo ($vo["invoice_id"]); ?>"><?php endif; ?>
                                        </td>
    ...
    <script type="text/javascript">
        $(".btn-primary").click(function(){
            ...
            layer.confirm('是否确认要开票？', {icon: 3}, function(index){
                layer.close(index);
                ajax_request({
                   url:'/lawyer/Admin/User/ensure_invoice',
                   params:{invoice_id:invoice_id},
                   succshow:true,
                   redirect:'/lawyer/Admin/User/invoice',
                });
            });
        });
    </script>
