	...
	$("#loginout").click(function(){
        ...
        <div class="row wrapper border-bottom white-bg page-heading">
            <div class="col-lg-12">
                ...
                <div class="col-sm-10">
                    <form action="<?php echo U('topiclist');?>" class="form-inline" method="get">
                        <div class="form-group">
                        用户：<input type="text" class="form-control input-sm" id="title" placeholder="请输入用户名" name="words" value="<?php echo ($words); ?>" size="15">
                        </div>
                        <div class="form-group">
                        <label for="title" class="sr-only">创建时间&lt;</label>
                        创建时间：<input type="text" class="form-control input-sm" id="ctime" placeholder="开始时间" name="ctime" value="<?php echo ($ctime); ?>" size="10">
                        </div>至
                        <div class="form-group">
                        <label for="title" class="sr-only">创建时间&gt;</label>
                        <input type="text" class="form-control input-sm" id="stime" placeholder="结束时间" name="stime" value="<?php echo ($stime); ?>" size="10">
                        </div>
				...
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="long-td">
                                                                               ...
                                                                               <td class="mtphotos">
                                                                                <?php if(is_array($vo["pictures"])): foreach($vo["pictures"] as $key=>$pic): ?><a href="<?php echo ($pic); ?>" data-lightbox="group<?php echo ($vo["tid"]); ?>"><img src="<?php echo ($pic); ?>"  width="60" height="60" /></a><?php endforeach; endif; ?>
                                                                                </td>
                                                                                <td><?php echo (date('Y/m/d H:i',$vo["ctime"])); ?></td>
                                                                                <td style="text-align:left;line-height: 2.5">
                                                                                    <a href="javascript:;" data-tid="<?php echo ($vo["tid"]); ?>" class="btn-sm btn-primary comments">
                                                                                                    评论</a>
                                                                                    <?php if($vo['isrcmd'] == '0'): ?><a href="javascript:;" onclick="rcmd(<?php echo ($vo["tid"]); ?>,1,'确认要推荐该话题?')" class="btn-sm btn-primary">
                                                                                            推荐</a>
                                                                                        <?php else: ?>
                                                                                        <a href="javascript:;" onclick="rcmd(<?php echo ($vo["tid"]); ?>,0,'确认要取消推荐该话题?')" class="btn-sm btn-primary">
                                                                                            取消推荐</a><?php endif; ?><br/>
                                                                                    <a href="javascript:;" data-id="<?php echo ($vo["tid"]); ?>" class="btn-sm btn-danger del">
                                                                                                    删除</a>
										</td>
...
<script>
    ...
    function rcmd(id,state,msg){
        layer.confirm(msg, {icon: 3}, function(index){
            $.post("<?php echo U('rcmdtopic');?>",{tid:id,state:state},function(data){
    ...
    $(function(){
        $(".del").click(function(){
            ...
            layer.confirm('确定删除该条话题？', {icon: 3}, function(index){
                $.getJSON('<?php echo U('deltopic','','');?>/tid/'+id,function(data){
                    layer.msg(data.msg,{skin:"layui-layer-hui",time:1000,end:function(){
                        if(data.status == 0){
                            window.location.reload();
                        }
