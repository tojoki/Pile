...
<!--                                    <th width="10%">强制升级</th>-->
							...
							<tbody>
								<?php if(is_array($list)): $key = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr class="long-td">
                                        <td><?php echo ($key); ?></td>
...
<!--                                        <td>
                                        <?php if($vo["force"] == '1'): ?><strong style="color:red;">是</strong>
                                        <?php else: ?>
                                            <strong style="color:green;">否</strong><?php endif; ?>
                                        </td>-->
                                       <td>
                                        <?php
 if($vo['type_id']==1){ echo "用户端"; }else if($vo['type_id']==2){ echo "洗车端"; }else if($vo['type_id']==3){ echo "美容美发端"; } ?>
                                       </td>
                                       <td><?php echo (date('Y-m-d H:i:s',$vo["ctime"])); ?></td>
										<td>
											<a href="javascript:;" data-id="<?php echo ($vo["vid"]); ?>" class="btn btn-primary edit">
												<i class="fa fa-pencil"></i> 详细</a>&nbsp;&nbsp;
											<a href="javascript:;" data-id="<?php echo ($vo["vid"]); ?>" class="btn btn-danger del">
...
<script>
    ...
    <script type="text/javascript">
    $(function(){
        var url = "<?php echo U("versionadd",'','');?>";
        var upd_url = "<?php echo U("versionedit",'','');?>";
        ...
        $(".del").click(function(){
            ...
            layer.confirm('确定删除？', {icon: 3}, function(index){
                $.getJSON('<?php echo U('versiondel','','');?>/vid/'+id,function(data){
