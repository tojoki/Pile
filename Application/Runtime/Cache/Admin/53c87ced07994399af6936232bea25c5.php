						...
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($list)): $key = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr class="long-td">
                                        <td><?php echo ($key); ?></td>
                                        <td><?php echo ($vo["worker_name"]); ?></td>
                                        <td><?php echo ($vo["phone"]); ?></td>
                                        <td><?php echo ($vo["title"]); ?></td>
                                        <td>
                                            <?php if($vo['cate_id'] == 1): ?>洗车服务<?php endif; ?>
                                            <?php if($vo['cate_id'] == 2): ?>美容美发服务<?php endif; ?>
                                        </td>
										<td>
                                            <a href="javascript:;" data-id="<?php echo ($vo["worker_id"]); ?>" class="btn-sm btn-primary edit">
                                                <i class="fa fa-pencil"></i> 修改</a>
											<a href="javascript:;" data-id="<?php echo ($vo["worker_id"]); ?>" class="btn-sm btn-danger del">
    ...
    $(function(){
        ...
        $(".del").click(function(){
            ...
            layer.confirm('确定删除？', {icon: 3}, function(index){
                $.getJSON('<?php echo U('deel','','');?>/id/'+id,function(data){
